...
public class TestCase23 {
	...
	static Logger log = Logger.getLogger(TestCase23.class);

    @Given("Launch browser and navigate to application_16")
    public void launch_browser_and_navigate_to_application_16() {
    ...
    public void enter_valid_username_16(DataTable dataTable) throws IOException {
    ...
    public void enter_valid_password_16(DataTable dataTable) throws IOException {
		List<Map<String,String>> credentials = dataTable.asMaps(String.class,String.class);
		for(Map<String,String> row : credentials)
		{
    ...
    @When("Click on login button_16")
    public void click_on_login_button_16() {
		Login login = new Login(driver);
		login.verifyLoginButton();
		Assert.assertTrue(login.flag);
    }
    @Then("Add products to the cart_16")
    public void add_products_to_the_cart_16() {
		Dashboard dashboard = new Dashboard(driver);
		dashboard.verifyAddToCart();
		Assert.assertTrue(dashboard.flag);
    }
    @Then("Click on cart button_16")
    public void click_on_cart_button_16() throws IOException, InterruptedException {
    	Dashboard dashboard = new Dashboard(driver);
    	dashboard.verifyCartButton();
    	Assert.assertTrue(dashboard.flag);
    }
    @Then("Click on checkout button")
    public void click_on_checkout_button() throws SocketException {
