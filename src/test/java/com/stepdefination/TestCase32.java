...
public class TestCase32 {
    ...
    public void Launch_browser_and_navigate_to_application_25() {
    ...
    public void Enter_valid_username_25(DataTable dataTable) throws IOException {
    ...
    public void Enter_valid_password_25(DataTable dataTable) throws IOException {
		List<Map<String,String>> credentials = dataTable.asMaps(String.class,String.class);
		for(Map<String,String> row : credentials)
		{
    ...
    @When("Click on login button_25")
    public void Click_on_login_button_25() {
		Login login = new Login(driver);
		login.verifyLoginButton();
		Assert.assertTrue(login.flag);
    }
    @Then("Add product to cart_25")
    public void Add_product_to_cart_25() {
		Dashboard dashboard = new Dashboard(driver);
		dashboard.verifyAddToCart();
		Assert.assertTrue(dashboard.flag);
    }
    @Then("Click on cart button_25")
    public void Click_on_cart_button_25() throws IOException, InterruptedException {
		Dashboard dashboard = new Dashboard(driver);
		dashboard.verifyCartButton();
		Assert.assertTrue(dashboard.flag);
    }
    @Then("Click on checkout button_25")
    public void Click_on_checkout_button_25() {
		Dashboard2 dashboard2 = new Dashboard2(driver);
		dashboard2.verifyCheckoutButton();
		Assert.assertTrue(dashboard2.flag);
    }
    @Then("Click on continue button_25")
    public void Click_on_continue_button_25() {
    	Dashboard3 dashboard3 = new Dashboard3(driver);
    	dashboard3.verifyContinueButton();
    	Assert.assertTrue(dashboard3.flag);
    }
    @Then("Verify user is on Checkout: Overview page_25")
    public void Verify_user_is_on_checkout_overview_page_25() {
    	Dashboard3 dashboard3 = new Dashboard3(driver);
